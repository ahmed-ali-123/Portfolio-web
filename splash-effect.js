// Global variables for the splash effect
let canvas, gl, ext;
let config = {
  SIM_RESOLUTION: 128,
  DYE_RESOLUTION: 1440,
...
function generateColor() {
  // Generate mostly white/light colors with slight tints
  const hue = Math.random(); // Random hue for slight color variation
  const saturation = 0.05 + Math.random() * 0.15; // Very low saturation (0.05-0.2) for subtle tint
  const brightness = 0.8 + Math.random() * 0.2; // High brightness (0.8-1.0) for white/light colors
  
  let c = HSVtoRGB(hue, saturation, brightness);
  c.r *= 0.3; // Slightly higher multiplier for visibility
  c.g *= 0.3;
  c.b *= 0.3;
  return c;
}
...
function initSplashEffect() {
  ...
  if (!canvas) {
    console.error('Canvas element with id "splash-canvas" not found');
    return;
  }

  console.log('Initializing splash effect...');
  ...
  console.log('Splash effect initialized successfully');
}

window.addEventListener('DOMContentLoaded', initSplashEffect);